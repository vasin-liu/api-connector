...
<template>
  <section class="panel">
    <router-link class="back-link" :to="`/connectors/${code3rd}`">← 返回配置</router-link>
    <div class="page-header">
      <h1>Invoke Lab · {{ code3rd }}</h1>
      <p class="subtitle">按 Spec 端点试调，Query / Body 将透传至运行时引擎</p>
    </div>
    ...
    <div class="actions">
      <button class="btn" :disabled="loading" @click="runTrial">
        {{ loading ? '传输中…' : '▶ 发送请求' }}
      </button>
    </div>
