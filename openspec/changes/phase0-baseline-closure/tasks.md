...
- [ ] 4.1 Open one PR from `feat/v2.7-greenfield-runtime` (engine + this change’s doc commits) into `main` and verify GitHub shows the PR URL with workflow `v2-7-runtime` attached
...
- [ ] 5.1 State in the PR body that the next OpenSpec change is `protocol-validation-wenxin-gaode` (文心 OAuth + 高德交通 HMAC) with no Host/admin in that follow-up, and verify the PR description contains that sentence
